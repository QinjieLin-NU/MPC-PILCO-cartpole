...
np.random.seed(0)
from utils import policy, rollout, Normalised_Env,make
...
env = Normalised_Env('CartPoleSwingUpContinuous', np.mean(X1[:,:4],0), np.std(X1[:,:4], 0))
X = np.zeros(X1.shape)
...
R = ExponentialReward(state_dim=state_dim,
                      t=np.divide([0.5,0.0,0.0,0.0] - env.m, env.std),
                      W=np.diag([0.5,0.1,0.1,0.1])
                     )
